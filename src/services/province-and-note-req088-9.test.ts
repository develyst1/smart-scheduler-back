...
describe("🚫 §9.1 — what this task deliberately does NOT do", () => {
  test("no cleanup of existing rows — no script, no migration touched", async () => {
    ...
    expect((journal.match(/"tag"/g) ?? []).length).toBe(41); // 🔻 0035 … 0040 added since (rentals, users, roles, course rental marker, student archive, other schedule) — none a province script
