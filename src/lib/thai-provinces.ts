// TASK-352 (`REQ-088 §9`) — the 77 provinces, FULL FORM, as the admin form's list spells them.
//
// 🔑 `parents.province` holds the PROVINCE ONLY, and it must be one of these — the report groups on it. A page
// or a client that sends anything else is refused (`PROVINCE_UNKNOWN`) rather than allowed to split the column
// into `กทม` / `กรุงเทพ` / `กรุงเทพมหานคร`. 📌 The FE's dataset pins EQUAL to this list; this is the source.
// 🚫 No abbreviations, no English, no trailing "จังหวัด" — the name as it is written on the form.
...
const SET = new Set(THAI_PROVINCES);

/** Exact match on the full form. 🚫 No trimming of "จังหวัด", no fuzzy match — a near-miss is a refusal. */
export const isThaiProvince = (name: string | null | undefined): boolean => !!name && SET.has(name.trim());
