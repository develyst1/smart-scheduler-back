...
// the 4th reason; the FAMILY's NEW notice as a placeholder kind; the other teachers' coach notice). 55 = 55.
...
describe("🔴 the migration — 0044, counted, ONE nullable FK column + the partial unique index LAST = the witness (source)", () => {
  ...
  test("55 = 55: `0044_user_teacher_link` is the 45th file, idx 44 (TASK-410/411 added 0045/0046 after it); 'expects 45' in the header", () => {
...
describe("🔴 OWN SCOPE — the link is the scope (fails closed); ONE predicate; 404 outside by id (value + source)", () => {
  ...
  test("by source: every scoped read names the ONE predicate; no read restates `teacherId = me`; the by-id reads and the status write assert it at the route", () => {
    ...
    // 🔻 TASK-454 — the camp weeks are read ONCE (the hour cells take their kid count from the same numbers); the
    // scope filter is unchanged, it just filters the read instead of re-reading.
    expect(cal).toContain("const campWeeks = await weeksForCalendar(range);");
    expect(cal).toContain("campWeeks.filter((w: any) => !scope || (w.teacherIds ?? []).includes(scope))"); // my camp weeks
  ...
  test("🔴 TASK-410 — the ONE family sender by VALUE through a fake tx: CONFIRMED ⇒ one outbox row per linked device carrying the code; an unlinked family ⇒ a SKIPPED row; a GROUP ⇒ every seat's family; PENDING ⇒ nothing", async () => {
    ...
    const tx = (o: { seats?: { studentId: string }[]; parents: Record<string, { lineUserId: string | null; links: string[] }>; student: Record<string, string> }) => ({
      ...
      query: {
        ...
        students: { findMany: async () => Object.entries(o.student).map(([id, parentId]) => ({ id, parentId })), findFirst: async ({ where }: any) => { const probe: string[] = []; where({ id: "id" }, { eq: (_: any, v: string) => { probe.push(v); return null; } }); const sid = probe[0]!; return o.student[sid] ? { id: sid, parentId: o.student[sid] } : null; } },
