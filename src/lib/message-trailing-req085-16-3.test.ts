...
describe("✅ TASK-325 §2 — ONE trim, at the builder's exit", () => {
  ...
  test("🚫 the five per-branch `.trimEnd()`s are GONE — asserted as an absence so a new branch cannot re-add one", () => {
    ...
    expect(switchBody.match(/case "/g)!.length).toBe(26); // 🔻 TASK-508: +1 (`class_on_again_teacher`) // 🔻 TASK-443: +1 (`camp_deduction`) // 🔻 TASK-441: +1 (`weekly_schedule_teacher`) // 🔻 TASK-428: +3 (the OTHER SERIES notices) // TASK-403 +1, TASK-406 +1 (`class_cancelled_parent`) // TASK-370: +2, TASK-375: +1 — see the kind-list pin // …and it really is the whole switch
