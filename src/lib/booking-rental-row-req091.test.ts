...
describe("🔴 the migration — 0035, counted, witnessed, the lock named (source)", () => {
  ...
  test("40 = 40 (TASK-377 added 0036, TASK-387 added 0037, TASK-390 added 0038, TASK-392 added 0039): the 36th file is `0035_booking_rentals`, at idx 35", () => {
    expect(files.length).toBe(40);
    expect(files[35]).toBe("0035_booking_rentals.sql"); // 🔻 TASK-377: 0036_users is the 37th
    expect((JOURNAL.match(/"tag"/g) ?? []).length).toBe(40);
