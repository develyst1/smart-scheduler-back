...
// swap route, `CAMP_ROW_OWNED` at every human row write (a table), the reminder fold, no money by absence. 49 = 49.
...
describe("🔴 the migration — 0047, counted, the day table + the window + the link + the partial index LAST = the witness; the SHARE lock named (source)", () => {
  ...
  test("49 = 49: `0047_camp_week_days` is the 48th file, idx 47 (TASK-420 added 0048 after it); 'expects 48'", () => {
...
describe("🔴 the kinds — CAMP is the 4th; the type ⇔ kind pin knows it; the three HUMAN validators refuse it (CAMP only via the sync)", () => {
  test("by value", () => {
    ...
    expect((code(src("src/validation.ts")).match(/z\.enum\(HUMAN_OTHER_KINDS\)/g) ?? []).length).toBe(3);
  });
});
