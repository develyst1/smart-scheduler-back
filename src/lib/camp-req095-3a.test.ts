...
// the four sale items, the RBAC keys (13 menus / 54 acts), the nine routes through the ROOT app. 56 = 56.
...
describe("🔴 the migration — 0042, counted, witnessed by the UNIQUE index, no expiry BY STRUCTURE, the `students` lock named (source)", () => {
  ...
  test("56 = 56: `0042_camp` is the 43rd file, idx 42 (TASK-403 added 0043 after it); the order 0038 → 0042 named in the header", () => {
...
describe("🔑 RBAC — `menu:camp` after Badges (13), four `action:camp.*` (54), ten rows in the access table (TASK-403 added the QR)", () => {
  test("keys by value", () => {
    ...
    expect(ACTION_REGISTRY.length).toBe(60); // 🔻 TASK-431: + bookings.coach-rate // 🔻 TASK-428: + calendar.other-cancel-all // 🔻 TASK-426: + teachers.budget-view // TASK-411: + people.parent-archive // TASK-406: + calendar.teacher-leave
