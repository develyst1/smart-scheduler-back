...
describe("📌 TASK-327 — WHICH branches are safe, and HOW. Two mechanisms, and only one of them scales.", () => {
  ...
  test("🚫 ZERO product-code changes — the reason this is safe during a `uat` round", () => {
    ...
    expect(switchBody.match(/case "/g)!.length).toBe(25); // 🔻 TASK-443: +1 (`camp_deduction`) // 🔻 TASK-441: +1 (`weekly_schedule_teacher`) // 🔻 TASK-428: +3 (the OTHER SERIES notices) // TASK-403 +1, TASK-406 +1 (`class_cancelled_parent`) // TASK-370: +2, TASK-375: +1 — see the kind-list pin
  });
});
