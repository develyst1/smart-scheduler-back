...
describe("🔴 the source — actor = username everywhere, the env login retired, the guard, the migration", () => {
  test("🔑 zero `.sub` actors: every actor site in the router is `actorOf(c)` — 14 of them — and no route reads `c.get(\"user\")?.sub`", () => {
    ...
    expect((API.match(/actorOf\(c\)/g) ?? []).length).toBe(17); // 🔻 TASK-428: + `POST /other-series/:key/cancel-all` // 🔻 TASK-411: + `POST /parents/:id/archive` // 🔻 TASK-406: + `POST /teachers/me/leave` // 🔻 TASK-390: + `DELETE /courses/:id/rental`; 🔻 TASK-392: + `POST /students/:id/archive`
  ...
  test("🔴 50 = 50 (0037 … 0049 added since): `0036_users` is the 37th file, idx 36; two tables, the UNIQUE on user_permissions LAST; the witness; the lock sentence", () => {
