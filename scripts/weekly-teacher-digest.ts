...
//   bun build --compile scripts/weekly-teacher-digest.ts --outfile dist/weekly-teacher-digest
// Windows Task Scheduler: run dist/weekly-teacher-digest.exe daily at 08:15, "Run whether user logged on or not".
...
if (!secret) {
  console.error("[weekly-teacher-digest] INTERNAL_JOB_SECRET is not set — aborting");
  process.exit(2);
}

try {
  const res = await fetch(`${base}/internal/jobs/weekly-teacher-digest`, {
    method: "POST",
  ...
  if (!res.ok) {
    console.error(`[weekly-teacher-digest] ${stamp} FAILED ${res.status}: ${text}`);
    process.exit(1);
  }
  console.log(`[weekly-teacher-digest] ${stamp} OK: ${text}`);
} catch (err) {
  console.error(`[weekly-teacher-digest] ${new Date().toISOString()} ERROR:`, err);
  process.exit(1);
}
